...
#Loading the collisions dataset available from "https://data.cityofnewyork.us/Public-Safety/NYPD-Motor-Vehicle-Collisions-Crashes/h9gi-nx95/data"
raw_df<-read.csv(file="/Users/neelampatodia/Downloads/collisions_data.csv", header=TRUE, sep=",")
```
...
reasons <- c("Driver Inattention/Distraction", "Failure to Yield Right-of-Way", "Traffic Control Disregarded","Following Too Closely", "Fatigued/Drowsy", "Backing Unsafely")

df<-raw_df %>% 
  select (TIME,BOROUGH,LATITUDE,LONGITUDE,NUMBER.OF.PERSONS.INJURED, CONTRIBUTING.FACTOR.VEHICLE.1) %>%
...
```{r}
#..............#Summary of DataSet
#accidents count by borough
ggplot(data=df, aes((BOROUGH))) + 
...
ggplot(data=df, aes((CONTRIBUTING.FACTOR.VEHICLE.1))) + 
...
-> Using maps have thus enabled us to seamlessly identify patterns in the data by accomodating for multiple parameters into one graph. For example, it clearly shows outliers due to incorrect data capture or similar parameters as vehicle road collisions cannot take place in water (denoted by green and purple dots on black background). This particular information could have been easily missed in the earlier graphs and could have adversaely affected our insights. We also see, that more number of accidents occur around 5pm in bottom right regions. 
-> Overall, it offers a better visualization of spatial data as opposed to normal graphs. 
