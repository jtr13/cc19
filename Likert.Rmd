...
```{r, include=FALSE}
knitr::opts_chunk$set(fig.align = 'center', out.width = '60%', warning = FALSE, message = FALSE)
```
...
```{r first-bar, echo=FALSE, out.width='75%'}
library(ggplot2)
library(tidyverse)
...
And here's the code:
```{r, ref.label="first-bar", eval=FALSE}
```
...
```{r second-bar, echo=FALSE, out.width='75%'}
...
```
And here's the code:
```{r, ref.label="second-bar", eval=FALSE}
```
